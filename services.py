...
async def create_lead(user: _schemas.User, db: _orm.Session, lead: _schemas.Book):
    lead = _models.Book(**lead.dict())
    db.add(lead)
    db.commit()
    db.refresh(lead)
    return _schemas.Book.from_orm(lead)


async def get_leads(user: _schemas.User, db: _orm.Session):
    leads = db.query(_models.Book).filter_by(owner_id=user.id)

    return list(map(_schemas.Book.from_orm, leads))
...
async def _lead_selector(lead_id: int, user: _schemas.User, db: _orm.Session):
    lead = (
        ...
        .filter(_models.Book.id == lead_id)
        .first()
    )
...
async def update_lead(lead_id: int, lead: _schemas.Book, user: _schemas.User, db: _orm.Session):
    lead_db = await _lead_selector(lead_id, user, db)

    lead_db.title = lead.title
    lead_db.author = lead.author
    lead_db.pages = lead.pages
    lead_db.in_stock = lead.in_stock
